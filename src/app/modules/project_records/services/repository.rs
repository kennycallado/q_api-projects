...
pub async fn get_all(db: &Db) -> Result<Vec<ProjectRecord>, diesel::result::Error> {
    let project_records = db.run(move |conn| {
...
pub async fn get_by_project_id(db: &Db, project_id: i32) -> Result<Vec<ProjectRecord>, diesel::result::Error> {
    let project_records = db.run(move |conn| {
        project_records::table
...
pub async fn create(db: &Db, new_project_records: NewProjectRecord) -> Result<ProjectRecord, diesel::result::Error> {
    let project_records = db.run(move |conn| {
...
pub async fn update(db: &Db, id: i32, new_project_records: NewProjectRecord) -> Result<ProjectRecord, diesel::result::Error> {
    let project_records = db.run(move |conn| {
